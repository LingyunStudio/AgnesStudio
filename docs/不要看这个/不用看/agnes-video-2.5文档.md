...
<Info>
  Agnes Video 2.5 已在国际站上线，使用异步视频生成 API。先调用 `POST /v1/videos` 创建任务，再使用返回的 `id` 调用 `GET /agnesapi?video_id=<VIDEO_ID>` 查询进度和结果。当前优惠规则请参阅下方“计费规则”。
</Info>
...
| 参考图片 | 前 5 张免费；从第 6 张开始，超出部分按 `$0.005 / 张` 计费。  |

<Note>
  当前支持 720P，公式中的“输出视频分辨率单价”按 720P 刊例价 `$0.025 / 秒` 计算。
</Note>
...
| 计费项    | 限免期间                             |
| ------ | -------------------------------- |
| 输出视频秒数 | 正常计费，按 720P 刊例价 `$0.025 / 秒` 计算。 |
| 输入视频秒数 | `$0`，暂不计费。                       |
...
假设生成一个 8 秒的 720P 视频，使用 3 秒输入视频和 7 张参考图片，并按 720P 刊例价 `$0.025 / 秒` 计算：

```text theme={null}
正式费用 =（8 + 3）× $0.025 + max(0, 7 - 5) × $0.005
         = $0.275 + $0.01
         = $0.285
