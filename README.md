...
### 通用
- **API Key 管理**：点击顶栏 **⚙** 打开设置，填写 Bearer Token，可显示/隐藏，保存在本机配置文件
- **主题与语言**：设置中可切换 浅色 / 深色 / 跟随系统 与 中文 / English，顶栏也可快捷切换
- **保存目录**：可自定义并记忆，支持浏览选择
- **历史记录**：生成过的图像/视频可在底部历史栏快速回看（仅占主区，不影响左侧生成按钮）
- **状态指示**：实时显示 API Key 状态与生成进度；报错统一展示在主生成区，一眼可见
...
│   ├── config.rs      # 本地配置读写
│   ├── theme.rs       # 主题系统：浅色/深色/跟随系统（液态玻璃样式表）
│   ├── i18n.rs        # 中英双语文案
...
1. 启动软件后，点击顶栏 **⚙** 打开「设置」，填写你的 **API Key**（Bearer Token），点击「保存设置」。
2. 顶部切换「🖼 图片」或「🎬 视频」工作区。
3. 选择模型、输入提示词、选择尺寸，点击侧栏底部的「生成」按钮（固定常驻，无需滚动）。
